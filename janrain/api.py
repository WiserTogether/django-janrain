...
def unmap(pk, identifier=None, all_identifiers=False, unlink=False):
    if identifier and all_identifiers:
        return False
...
def _api_call(function, **kwargs):
    ...
    api_url = 'https://rpxnow.com/api/v2/%s' % function

    logger.debug(
        'JANRAIN: Sending API call to %s with params %s' % (
            api_url,
    ...
    try:
        ...
        js = response.read()
        data = json.loads(js)
        logger.debug('Recived the following data: %s' % data)
    except ValueError:
        logger.error(
            'JANRAIN: Data returned from Janrain API call is not JSON (%s).',
            js
        )
        ...
        logger.error(
            'JANRAIN: An unhandled exception occurred: %s' % str(e)
        )
    ...
    if 'profile' not in data.keys():
        raise JanrainAuthenticationError(
            'JANRAIN: No profile data was returned from Janrain.'
        )
